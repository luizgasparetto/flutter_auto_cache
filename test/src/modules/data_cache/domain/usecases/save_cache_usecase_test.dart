...
void main() {
  ...
  group('SaveCache |', () {
    ...
    test(
      ...
      () async {
        when(
          () => repository.get<String>(any(that: _cacheDtoMatcher())),
    ...
    test(
      'should NOT be able to save cache repository when get fails',
      () async {
        when(
          () => repository.get<String>(any(that: _cacheDtoMatcher())),
        ).thenAnswer(
    ...
    test(
      ...
      () async {
        when(() => repository.get<String>(any(that: _cacheDtoMatcher()))).thenAnswer(
