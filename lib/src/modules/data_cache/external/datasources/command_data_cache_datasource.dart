...
final class CommandDataCacheDatasource implements ICommandDataCacheDatasource {
  ...
  const CommandDataCacheDatasource(this.storageService, this.cryptographyService);

  @override
  Future<void> write<T extends Object>(DataCacheEntity<T> cache) async {
    final encryptedData = getEncryptedData<T>(cache);

    await storageService.save(key: cache.id, data: encryptedData);
  }

  @override
  Future<void> clear() => storageService.clear();

  @override
  Future<void> delete(String key) => storageService.delete(key: key);
